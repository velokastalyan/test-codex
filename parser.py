...
Сохраняет в sprint_rowery_<stamp>.csv и sprint_rowery_<stamp>.xlsx
...
HEADERS   = {"User-Agent": "Mozilla/5.0"}   # обычный ASCII дефис
...
def fetch_category(url: str) -> str:
    ...
    if len(crumbs) >= 2:
        # берём ПРЕДпоследний элемент (последний — это сам товар)
...
def parse_page(url: str) -> tuple[list[dict], str | None]:
    print(f"[+] обрабатываем: {url:<80}", end=" | ")
...
def save(data: list[dict]):
    ...
    print(f"✅ Файл(ы) сохранены: {csv_name}, {xlsx_name}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if collected:
            save(collected)
            print(f"✔️ Сохранено {len(collected)} товаров.")
        else:
